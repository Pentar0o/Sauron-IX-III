...
def draw_tracks(frame: np.ndarray, cars: OrderedDict[int, TrackableObject], colors: list[tuple[int,int,int]]):
    ...
    Args:
        ...
        colors (list): List of colors for each day.
    """
    # loop over the tracked objects
    for object_id, car in cars.items():
        rect = car.rect
        time_since_parked = time.time() - car.apparition
        days = time_since_parked // (24 * 60 * 60)
        colors_index = min(int(days), len(colors) - 1)
        #print(f"Car {object_id} has been parked for {days} days")
        box_label(frame, rect, label=f"Car {object_id}", color=colors[colors_index])
