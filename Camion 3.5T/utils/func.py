...
def get_list_cam(path: str) -> tuple:
    ...
    Returns:
        tuple: Webhook and list of cameras
    """    
    if os.path.exists(path):
        with open(path,"r") as f:
            list_cam = json.load(f)
            webhook = list_cam.get("Teams_webhook", None)
            cameras = list_cam.get("Cameras", [])
            if webhook is None:
            ...
            if len(cameras) == 0:
                ...
                sys.exit(-1)
            return webhook, cameras

    else:
        print("File Not Found")
        sys.exit(-1)

def parse_aws_json(path: str) -> dict:
    """ Parse a JSON file to get the AWS configuration
        exit if the file is empty or not found
